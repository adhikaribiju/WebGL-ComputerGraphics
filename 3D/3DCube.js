...
var FSHADER_SOURCE = `
    ...
    uniform vec4 color;
  void main(){
    //gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); // set the fragment color to black
    gl_FragColor = color;
    }
`;
...
for (let i =0; i< numSides; i++){
  ...
  // Draw each side of the cube using correct vertex indices
  //gl.drawArrays(gl.LINE_LOOP, 0, 4);   // Side A
  //gl.drawArrays(gl.LINE_LOOP, 4, 4);   // Side B
  //gl.drawArrays(gl.LINE_LOOP, 8, 4);   // Side C
  //gl.drawArrays(gl.LINE_LOOP, 12, 4);  // Side D
  //gl.drawArrays(gl.LINE_LOOP, 16, 4);  // Side E
  //gl.drawArrays(gl.LINE_LOOP, 20, 4);  // Side F
}
