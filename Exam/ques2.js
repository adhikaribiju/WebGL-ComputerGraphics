...
function changeColors() {
    ...
    var color1RGB = colorMap[color1];
    var color2RGB = colorMap[color2];
    ...
    FSHADER_SOURCE = `
        ...
        void main() {
            if (mod(floor(v_StackIndex) + floor(v_SliceIndex), 2.0) < 1.0) { 
                gl_FragColor = vec4(${color1RGB[0]}, ${color1RGB[1]}, ${color1RGB[2]}, 1.0);
            } else {
                gl_FragColor = vec4(${color2RGB[0]}, ${color2RGB[1]}, ${color2RGB[2]}, 1.0);
            }
        }
    `;

    // Seting flag to reinitialize shaders
