...
var angleX = 0;
var angleY = 0;
var rotationSpeed = 0.005; // Default rotation speed
...
var FSHADER_SOURCE = `
    ...
    void main() {
        if (mod(v_StackIndex, 2.0) < 1.0) {
            gl_FragColor = vec4(1.0, 1.0, .0, 1.0); // Yellow
        } else {
...
function loadVerticesAndIndices() {
    for (var i = 0; i <= stacks; i++) {
        ...
        for (var j = 0; j <= slices; j++) {
            ...
            vertices.push(radius * x, radius * y, radius * z, i); // Stack index as 4th value
        }
    }
...
function main() {
    ...
    gl.vertexAttribPointer(a_StackIndex, 1, gl.FLOAT, false, 16, 12);
    ...
    speedSlider.addEventListener("input", function() {
        rotationSpeed = speedSlider.value / 10000;
    });
...
function render() {
    ...
    // Drawing elements with yellow and black stripes
